...
from model.encoder.encoder import get_encoder_model_raw
from model.encoder.encoder_args_core import dataset_params, model_params, run_params 
from common.optimizers import CustomSchedule
from datasets import data_pipeline_encoder 
...
if __name__ == '__main__':
    ...
    vocab_size = data_pipeline_encoder.VOCAB_SIZE # 输入输出是一致的, 必须要+2， 输入padding的时候多了2个字符

    print_log = Print_log(log_file) 
    print_log("\n".join(["\n", str(model_params), str(run_params), str(dataset_params), "\n"]))

    model = get_encoder_model_raw(dataset_params['padding_input_max_length'], 
        ...
        rate=0.1)
    # lr
    learning_rate = 0.002 # CustomSchedule(model_params['d_model'], warmup_steps=500) 
    ...
    # dataset
    dataset = data_pipeline_encoder.train_input_fn(
